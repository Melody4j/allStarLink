...
class AllStarLinkParser:
    """Parses AllStarLink detail payloads into existing project models."""
    ...
    def parse_node(self, data: Dict) -> Optional[Node]:
        try:
            ...
            return Node(
                ...
                total_keyups=int(
                    node_data.get("totalkeyups", DEFAULT_TOTAL_KEYUPS) or DEFAULT_TOTAL_KEYUPS
                ),
                total_tx_time=int(
                    node_data.get("totaltxtime", DEFAULT_TOTAL_TX_TIME) or DEFAULT_TOTAL_TX_TIME
                ),
    ...
    def _parse_hardware_type(self, site_name: str) -> str:
        ...
        if any(
            keyword in site_name_upper
            for keyword in HARDWARE_KEYWORDS[HARDWARE_TYPE_INFRASTRUCTURE]
        ):
    ...
    def _parse_connection_modes(self, connection_modes: str) -> Dict[str, str]:
        ...
        for item in connection_modes.split(","):
            ...
            if node_id:
                connection_dict[node_id] = CONNECTION_PREFIXES.get(
                    prefix, CONNECTION_DIRECTION_UNKNOWN
                )
